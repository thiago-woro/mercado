...
categoriesWithProducts = [100, 99, 98, 97, 96, 95,]  ///testing only
...
async function fetchAndCompareProducts(categoryNum, client) {
  ...
  try {
    ...
    if (newlyScrapedProducts.length > 0) {
      await compareAndSaveToDatabase(newlyScrapedProducts, client, "atacadao", 8000);
...
async function scrapeCategoriesLoop() {
  ...
  if (!client.topology.isConnected()) {
    ...
    //dont use this anymore because its already integrated to saveToDatabase() fn
